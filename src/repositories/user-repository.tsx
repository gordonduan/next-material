...
export class UserRepository {
        ...
        try {
            ...
            result.map(user => {
                let nextUser: NextUser = {
                    ...
                    createdDateTime: user.created_date_time as Date
                };
        ...
        try {
            ...
            if(result.length > 0){
                nextUser = {
                    ...
                    age: result[0].age as number,
                    email: result[0].email as string,
                    createdDateTime: result[0].created_date_time as Date
                };
            }
            return nextUser;
        } catch (error) {
    ...
    addUser = async(data: NextUser) => {
        try {
            var user  = {name: data.name, gender: data.gender, age: data.age, email: data.email, created_date_time: new Date()};
    ...
    updateUser = async(user: NextUser) => {
        try {
            const result = await dbQuery('UPDATE next_user SET name = ?, gender = ?, age = ?, email = ? WHERE id = ?', [user.name, user.gender, user.age, user.email, user.id]);
        } catch (error) {
            console.log(error);
        }
    }
